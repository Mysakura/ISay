...
<head>
<meta charset="UTF-8">
<title>互联网说-编辑-【${post.postTitle }】</title>
    ...
    <div class="row">
        ...
        <div class="col-xs-12">
            <div class="form-group">
                <input class="form-control" required type="text" id="postTitle" name="postTitle" form="poform" placeholder="请输入标题" value="${post.postTitle }">
            </div>
        </div>
        <div class="col-xs-12">
        ...
        <div class="col-xs-12">
            ...
            <ul class="list-unstyled">
                <c:forEach items="${area }" var="i">
                	<c:if test="${post.workArea.areaId == i.areaId }">
		                <li class="write-type write-type-active">
		                    <input type="radio" name="workArea.areaId" value="${i.areaId }" form="poform" id="a-${i.areaId }" checked="checked">
        ...
        <div class="col-xs-12">
            ...
            <ul class="list-unstyled">
            	<c:forEach items="${areaLabel }" var="i">
	            	<c:if test="${fn:contains(oldLabelStr,i.labelId) }">
		            	<li class="write-label write-label-active">
		                    <input type="checkbox" name="postLabel" form="poform" value="${i.labelId }" id="l-${i.labelId }" checked="checked">
		                    <label for="l-${i.labelId }">${i.labelName }</label>
		                </li>
	            	</c:if>
	            	<c:if test="${!fn:contains(oldLabelStr,i.labelId) }">
		            	<li class="write-label">
		                    <input type="checkbox" name="postLabel" form="poform" value="${i.labelId }" id="l-${i.labelId }">
		                    <label for="l-${i.labelId }">${i.labelName }</label>
		                </li>
	            	</c:if>
            	</c:forEach>
        ...
        <div class="col-xs-4 col-xs-offset-4">
            <input class="form-control submit-btn" type="submit" id="releasePost" value="保存修改" form="poform">
        </div>
        <div class="col-xs-12">
            <form action="${pageContext.request.contextPath }/card/save_edit_post" method="post" id="poform">
